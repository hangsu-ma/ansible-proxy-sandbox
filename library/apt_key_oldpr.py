...
options:
    ...
    keyserver_options:
        description:
            - The value for I(keyserver-options) parameter for apt-key advanced option.
        version_added: "2.7"
    state:
        description:
...
- name: Working behind a proxy, using keyserver_options
  apt_key:
    keyserver: keyserver.ubuntu.com
    keyserver_options: "http-proxy=http://username:password@proxy.example.com:8080"
...
def import_key(module, keyring, keyserver, keyserver_options, key_id):
    if (environ.get('http_proxy')):
        if environ.get('no_proxy') and in_no_proxy(module, keyserver, environ.get('no_proxy')):
            pass
        else:
            keyserver_options = "%s http-proxy=%s" % (keyserver_options if keyserver_options else '', environ.get('http_proxy'))
    if keyserver_options:
        cmd = "adv --keyserver-options '%s' --keyserver %s --recv %s" % (keyserver_options, keyserver, key_id)
    else:
        cmd = "adv --keyserver %s --recv %s" % (keyserver, key_id)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            keyserver_options=dict(type='str'),
    ...
    keyserver_options = module.params['keyserver_options']
    changed = False
    ...
    if state == 'present':
        if fingerprint and fingerprint in keys:
            ...
            if filename:
                ...
                import_key(module, keyring, keyserver, keyserver_options, key_id)
            else:
